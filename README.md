...
يوجد أيضاً سير عمل إصدار منفصل باسم **Publish NetRouter Release**. عند دفع tag مثل `v0.1.0`، أو عند تشغيله يدوياً مع tag بهذا الشكل، يبني ISO وEXE من المصدر ثم ينشرهما مباشرة في صفحة GitHub Releases، مع `netrouterd` و`netrouterctl` وملف `SHA256SUMS.txt`. لا ينشر هذا المسار أي ملف من حاسوب محلي؛ فكل الملفات المنشورة تُبنى في بيئة CI من commit الإصدار نفسه.
